...
int main() 
{
    ...
    int n = sizeof(cost[0])/sizeof(cost[0][0]) - 1;         // 32/4 - 1 
    // since it is a square matrix, cost[0] gives us the row size and dividing it with cost[0][0] 
    // (i.e. element size) gives us the no. of elements
    // and minus 1 because array size is always 0 to (n-1)

    // cout << sizeof(cost[0][0]) << endl;
    // cout << sizeof(cost[0]) << endl;
    PrimsMST(cost, n);
    return 0;
}