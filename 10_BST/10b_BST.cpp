...
private:
    Node* root;
public:
    BST(){ root = nullptr; }                // can also be written as NULL
...
Node* BST::rInsert(Node *p, int key) 
{
    ...
    if (p == nullptr)
    {
        ...
        t->lchild = nullptr;
        t->rchild = nullptr;
        // p = t;       no significant changes as the temporary parameter will be forgotten after coming out of function
        return t;
    }
...
int BST::Height(Node *p) 
{
    ...
    x = Height(p->lchild);
    y = Height(p->rchild);
    return x > y ? x + 1 : y + 1;       // if x > y then x+1 else y+1
}
